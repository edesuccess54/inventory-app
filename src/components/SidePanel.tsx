...
const SidePanel: React.FC = () => {
  ...
  return (
    <div className="hidden lg:block w-[230px] h-screen pb-8 bg-[#fff]">
      ...
      <nav className="flex flex-col justify-between mt-1 h-[calc(100vh_-_144px)]">
        <ul className="flex flex-col gap-2">
          {menuList.map(({ title, href, icon: ICON }) => (
            <Link
              key={title}
              className={`flex items-center gap-3 px-6 py-2 ${(isActive === href || isActive?.includes(href)) ? "text-[#1570EF]" : "text-text-dark"}`}
              href={href}
            >
              <ICON /> {title}
            </Link>
